module github.com/doertedev/formae-plugin-hcloud
...
// The formae SDK depends on the ergo actor framework via forks. Go does not
// inherit replace directives from dependencies, so re-declare them here. These
// point at published repos (not local paths) so the build is portable.
replace (
	ergo.services/actor/statemachine => github.com/JeroenSoeters/actor/statemachine v0.0.0-20260414171442-1ef660e4a3bc
	ergo.services/ergo => github.com/JeroenSoeters/ergo v1.999.320-pel.1
)
